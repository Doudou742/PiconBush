			...
			for(int j = 0; j < Plateau.grille[0].length; j ++){
				...
				if(Plateau.grille[i][j].contienRobot()){
					...
					if(Plateau.grille[i][j].getUnRobot().getEnergie() <= 0){
						JOptionPane.showMessageDialog(null, "L'equipe " + Plateau.grille[i][j].getUnRobot().getEquipe() + " a perdu un " + Plateau.grille[i][j].getUnRobot().getType() ,"Aie",JOptionPane.WARNING_MESSAGE);
						if(Plateau.grille[i][j].getUnRobot().getEquipe() == 1){
							nbMortA ++;
						}
       ...
       fichier = new BufferedReader(flux); 
		...
		if (choixEquipe1.equals("Sortir un robot de la base")) {
			...
			if(tableauEquipe1[0] == 0 && robotABouger.equals("Tireur")){
				JOptionPane.showMessageDialog(null, "Aucun Tireur dans la base de disponible!","Erreur",JOptionPane.ERROR_MESSAGE);
			}
			
			if(tableauEquipe1[1] == 0 && robotABouger.equals("Piegeur")){
				JOptionPane.showMessageDialog(null, "Aucun Piegeur dans la base de disponible!","Erreur",JOptionPane.ERROR_MESSAGE);
			}
			
			if(tableauEquipe1[2] == 0 && robotABouger.equals("Char")){
				JOptionPane.showMessageDialog(null, "Aucun Char dans la base de disponible!","Erreur",JOptionPane.ERROR_MESSAGE);
			}
			...
			if(tableauEquipe1[0]!=0 && robotABouger.equals("Tireur")){
				...
				if(Plateau.grille[1][0].estLibre()){
					...
					JOptionPane.showMessageDialog(null, "Aucune place de disponible autour de la base : bougez vos robots!","Erreur",JOptionPane.ERROR_MESSAGE);
				}
			...
			deplacement.agit();
				...
				JOptionPane.showMessageDialog(null, "Il n'y a pas de robot a cet endroit","Erreur",JOptionPane.ERROR_MESSAGE);
			}
		...
		if (choixEquipe2.equals("Sortir un robot de la base")) {
			...
			if(tableauEquipe2[0]!=0 && robotABouger.equals("Tireur")){
				...
				if(Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].estLibre()){ // l9|c8
					Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].addRobot(new Tireur(equipeB,2,Plateau.grille.length -1,Plateau.grille[0].length -2));
				}
				else if(Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].estLibre()){ // l8|c9
					Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].addRobot(new Tireur(equipeB,2,Plateau.grille.length -2,Plateau.grille[0].length -1));
				}
				else {
					JOptionPane.showMessageDialog(null, "Aucune place de disponible autour de la base : bougez vos robots!","Erreur",JOptionPane.ERROR_MESSAGE);
				}
			...
			if(tableauEquipe2[1]!=0 && robotABouger.equals("Piegeur")){
				...
				if(Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].estLibre()){ 
					Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].addRobot(new Piegeur(equipeB,2,Plateau.grille.length -1,Plateau.grille[0].length -2));
				}
				else if(Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].estLibre()){ 
					Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].addRobot(new Piegeur(equipeB,2,Plateau.grille.length -2,Plateau.grille[0].length -1));
				}
				else {
					JOptionPane.showMessageDialog(null, "Aucune place de disponible autour de la base : bougez vos robots!","Erreur",JOptionPane.ERROR_MESSAGE);
				}
			...
			if(tableauEquipe2[2]!=0 && robotABouger.equals("Char")){
				...
				if(Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].estLibre()){ 
					Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length - 2].addRobot(new Char(equipeB,2,Plateau.grille.length -1,Plateau.grille[0].length -2));
				}
				else if(Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].estLibre()){ 
					Plateau.grille[Plateau.grille.length -2][Plateau.grille[0].length -1].addRobot(new Char(equipeB,2,Plateau.grille.length -2,Plateau.grille[0].length -1));
				}
				else if(Plateau.grille[Plateau.grille.length -3][Plateau.grille[0].length -1].estLibre()){ 
					Plateau.grille[Plateau.grille.length -3][Plateau.grille[0].length -1].addRobot(new Char(equipeB,2,Plateau.grille.length -3,Plateau.grille[0].length -1));
				}
				else if(Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length -3].estLibre()){ 
					Plateau.grille[Plateau.grille.length -1][Plateau.grille[0].length -3].addRobot(new Char(equipeB,2,Plateau.grille.length -1,Plateau.grille[0].length -3));
				}
