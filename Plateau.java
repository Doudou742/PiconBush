...
public class Plateau {
	...
	private boolean bonPlacement() {
		for (int i = 0; i < plateau.length; i++) {
			for (int j = 0; j < plateau[0].length; j++) {
				if (i == 0 && j == 0) {
					if (!plateau[i + 1][j].estLibre() || !plateau[i][j + 1].estLibre()) { // autour
																							// dla
																							// base
																							// en
																							// haut
					...
					if (j == 9 && (!plateau[i + 1][j].estLibre() || !plateau[i][j - 1].estLibre())) { // coin
																										// en
																										// haut
																										// a
																										// droite
					...
					} else if (!plateau[i][j - 1].estLibre() || !plateau[i][j + 1].estLibre()
							|| !plateau[i + 1][j].estLibre()) { // premiere
																// ligne
						return false;
					} else if (j == 0) {
						if (j == 9 && (!plateau[i - 1][j].estLibre() || !plateau[i][j + 1].estLibre())) { // coin
																											...
																											// gauche
							return false;
						} else if (!plateau[i + 1][j].estLibre() || !plateau[i][j + 1].estLibre()
								|| !plateau[i - 1][j].estLibre()) { // premiere
																	// colonne
							return false;
						}
					} else if (i == plateau[0].length) { // derniere ligne
						if (!plateau[i][j - 1].estLibre() || !plateau[i][j + 1].estLibre()
								|| !plateau[i - 1][j].estLibre()) {
							return false;
						}
					} else if (j == plateau[0].length) { // derniere colonne
						if (!plateau[i + 1][j].estLibre() || !plateau[i][j - 1].estLibre()
								|| !plateau[i - 1][j].estLibre()) {
							return false;
						}
